    ...
    public class ActionClipInspector : Editor
    {
        ...
        protected void ShowSubClipGUI() {
            ...
            if ( subContainable != null && subContainable.subClipLength > 0 ) {
                GUILayout.Space(10);
                if ( GUILayout.Button("Set At Clip Length") ) {
                    action.length = subContainable.subClipLength / subContainable.subClipSpeed;
                }
            }
        }
        ...
        protected void ShowAnimatableParameters() {
            if ( action.hasParameters ) {
                foreach ( var animParam in action.animationData.animatedParameters ) {
                    if ( animParam.isProperty || animParam.isExternal ) { //field based parameters are shown through AnimatableParameterDrawer
