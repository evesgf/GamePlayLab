...
namespace Slate{
	...
	public class DirectorGUI : MonoBehaviour {
		...
		public float subtitlesSize = 18f;
		[Header("Overlay Text")]
...
/*
		public static void UpdateOverlaySprite(Sprite sprite, Color color, Vector2 scale, Vector2 positionOffset){
			if (OnSpriteOverlayGUI != null){
				OnSpriteOverlayGUI(sprite, color, scale, positionOffset);
				return;
			}
		}
*/
		...
		void DoSubs(){
			var finalSubs = string.Format("<size={0}><b><i>{1}</i></b></size>", subtitlesSize, subsText);
