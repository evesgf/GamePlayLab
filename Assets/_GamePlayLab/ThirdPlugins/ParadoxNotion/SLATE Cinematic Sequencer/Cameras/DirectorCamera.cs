...
namespace Slate{
	...
	public class DirectorCamera : MonoBehaviour, IDirectableCamera {
		...
		public static void Update(IDirectableCamera source, IDirectableCamera target, EaseType interpolation, float weight, float damping = 0f){
			...
			var endFocalPoint = weight < 1? Easing.Ease(interpolation, source.focalPoint, target.focalPoint, weight)	 	: target.focalPoint;
			var endFocalRange = weight < 1? Easing.Ease(interpolation, source.focalRange, target.focalRange, weight)	 	: target.focalRange;
			...
			if (!isCut && damping > 0){
				...
				current.focalPoint  = Mathf.Lerp(current.focalPoint, endFocalPoint, Time.deltaTime * (MAX_DAMP/damping));
				current.focalRange  = Mathf.Lerp(current.focalRange, endFocalRange, Time.deltaTime * (MAX_DAMP/damping));
			
			} else {
				current.position    = endPosition;
				current.rotation    = endRotation;
				current.fieldOfView = endFOV;
				current.focalPoint  = endFocalPoint;
				current.focalRange  = endFocalRange;
			}
